...
export const VideoShowcase = () => {
  ...
  // Defer video download until the section is near the viewport
  useEffect(() => {
    const el = sectionRef.current;
    if (!el) return;

    // Safety net for older browsers without IntersectionObserver
    ...
    const observer = new IntersectionObserver(
      ...
      { rootMargin: '400px 0px' } // start fetching 400px before the section enters view
    );
  ...
  // Once sources are mounted, trigger load() and attempt play()
  useEffect(() => {
    if (!shouldLoad) return;
    const v = videoRef.current;
    if (!v) return;
    // Defer slightly to ensure React has flushed the new <source> children to the DOM
    const id = requestAnimationFrame(() => {
      v.load();
      const playPromise = v.play();
      if (playPromise && typeof playPromise.catch === 'function') {
        playPromise.catch(() => {
          /* autoplay rejected — poster remains visible, harmless */
        });
      }
    });
          ...
          <div className="relative">
            ...
            >
              <div className="relative w-full" style={{ paddingBottom: '56.25%' }}>
                ...
                >
                  <source
                    src={shouldLoad ? '/assets/onpoint_quantum_financial_deployment.webm' : undefined}
                    type="video/webm"
                  />
                  <source
                    src={shouldLoad ? '/assets/onpoint_quantum_financial_deployment.mp4' : undefined}
                    type="video/mp4"
                  />
                </video>
